...
prob_ = []
for i in range(40):
  pre_trained_path = '../NF_ResNet/Checkpoints/checkpoints_DeepEnsemble_OpenSet_NF_ResNet_auc_entropyCifar6_' + str(i+1) + '.pt'   ##Your saved model path here
  ...
  for batch_idx, (data, target) in tqdm(enumerate(inlier_test_loader), total=len(inlier_test_loader), smoothing=0.9):        
      ...
      with torch.no_grad():
          ...
          probs = F.softmax(output, dim=-1)[:,[0,2,4],:].permute(1,0,2)  # [B, N, D]   --> [N, B, D]  
          prob_.append(probs)
      break
